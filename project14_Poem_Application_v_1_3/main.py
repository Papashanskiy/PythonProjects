...
class Application:
    ...
    def menu(self):
        ...
        for i in range(len(poem_name)):
            b = Button(self.root, text=poem_name[i], command=lambda j=poem_name[i]: self.choice_poem(j),
    ...
    def page(self, index):
        ...
        self.page_lbl = Label(self.root, text=index, font="helvetica 15")
        self.page_lbl.pack()

        text = self.poem_text(index)
        text = ''.join(text)
    ...
    def remove_poem(self):
        ...
        for i in range(len(poem_name)):
            b = Button(self.remove_frame, text=poem_name[i], command=lambda j=poem_name[i]: self.del_poem_cmd(j),
                       borderwidth=2, relief="groove")
            b.grid(row=i, column=1, padx=10, pady=10, sticky='W')
            self.btns.append(b)
    ...
    def add_btm_cmd(self):
        ...
        if name == "" or empty.group() == "":
            messagebox.showinfo("Error!", "Enter title of poem and text poem!")
        else:
    ...
    def del_poem_cmd(self, poem_del_name):
        with open('poems.json') as f:
            poems = json.load(f)

        for i in range(len(poems)):
            if poem_del_name in poems[i]['name']:
                point = i
                break

        poems.remove(poems[point])
